...
- The binding crate sets `test = false`. This disables Cargo's Rust test harness
  for that crate. The release instead relies on core-crate tests, Cargo checks,
  and installation and smoke tests of the native wheels.
...
- Twine and Maturin are direct development dependencies. Immutable action SHAs
  require Dependabot maintenance. The native matrix consumes CI minutes and
  artifact storage, and macOS and preview ARM runners can materially increase
...
- Some runner labels were obsolete or unstable, and action pinning was applied
  only to release workflows. Generated GitHub workflows now use supported runner
  labels and immutable action revisions consistently.
